...
class PLayersFilter(django_filters.FilterSet):
    ...
    team = django_filters.ChoiceFilter(
        choices=[(team.name, f"{team.thumbnail.url} {team.name}")  for team in Teams.objects.all()],
        field_name='team__name', 
        # lookup_expr='icontains', 
        label='Team', 
        widget=forms.Select(attrs={'class': 'form-select', 'id': 'team-select'})
    )
