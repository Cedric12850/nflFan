...
$(document).ready(function() {
    // Appliquer Select2 pour tous les champs avec la classe 'team-select'
    $('.team-select').select2({
        // Affichage des options dans le dropdown
        templateResult: function(data) {
            if (!data.id) {
                return data.text; // Si option vide, affiche le texte
            }

            // Récupération de l'image via data-image
            let img = $(data.element).data('image');

            // Retourne le HTML personnalisé avec l'image et le texte
            return $('<span><img src="' + img + '" style="width: 30px; height: 30px; margin-right: 10px;" />' + data.text + '</span>');
        },
