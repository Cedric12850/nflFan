...
$(document).ready(function () {
    $('.team-select').select2({
        templateResult: function (data) {
            ...
            return $(
                `<span><img src="${imgUrl}" style="width: inherit; height: auto; margin-right: 10px;" /> ${data.text}</span>`
            );
        },
        templateSelection: function (data) {
            const imgUrl = $(data.element).data('image');
            return $(
                `<span><img src="${imgUrl}" style="width: 30px; height: 30px; margin-right: 10px;" /> ${data.text}</span>`
            );
        }
    });
});