...
document.addEventListener("DOMContentLoaded", () => {
    fetch('/navbar/divisions/')
        .then(response => response.json())
        .then(data => {
            const selectElement = document.getElementById('id_team_local');  // Remplacez par l'ID de votre champ <select>
            ...
            data.forEach(division => {
                division.teams.forEach(team => {
                    ...
                    // Création de l'image de l'équipe
                    const teamImg = document.createElement('img');
                    teamImg.src = `/media/${team.thumbnail}`;  // Chemin vers l'image
                    teamImg.alt = `${team.name} logo`;
                    teamImg.style.display = 'inline-block'; // Assurez-vous que l'image est affichée correctement
teamImg.style.width = '30px'; // Taille de l'image
teamImg.style.height = '30px'; // Taille de l'image
teamImg.style.marginRight = '10px'; // Espacement à droite de l'image
teamImg.style.verticalAlign = 'middle'; // Aligner l'image avec le texte

                    // Création du texte pour l'option
                    const textNode = document.createTextNode(`${team.town} ${team.name}`);

                    // Ajout de l'image et du texte dans l'option
                    option.appendChild(teamImg); // L'image est ajoutée ici
                    option.appendChild(textNode); // Le texte est ajouté après l'image
