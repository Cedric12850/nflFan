...
def get_playgame_data():
    ...
    # Trouver la date du mercredi de la semaine en cours
    days_since_wednesday = (presentday.weekday() - 2) % 7  # Le jeudi est le jour 3 de la semaine
    start_of_week = presentday - timedelta(days=days_since_wednesday)

    end_of_week = start_of_week + timedelta(days=6)

# Convertir en datetime.date pour correspondre à week.start_date
start_of_week = start_of_week.date()  # Convertir en date
end_of_week = end_of_week.date()      # Convertir en date
    ...
    for week, games in playgames_by_week.items():
        ...
        if start_of_week <= week.start_date <= end_of_week:  # Assurez-vous que 'start_date' est un attribut valide
