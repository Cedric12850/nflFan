...
def get_team_results(team_id):
    ...
    """
    wins = PlayGame.objects.filter(team_local_id=team_id, score_local__gt=models.F('score_visitor')).count()
    wins += PlayGame.objects.filter(team_visitor_id=team_id, score_visitor__gt=models.F('score_local')).count()

    losses = PlayGame.objects.filter(team_local_id=team_id, score_local__lt=models.F('score_visitor')).count()
