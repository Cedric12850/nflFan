...
def get_division_classement(team_id):
    ...
    # tri des équipes par victoires
    sorted_teams = sorted(
        team_result,
        key=lambda x: (-x["team_wins"], x["team_losses"])
    )
    
    # position de l'equipe dans sa division
    ...
    return None  # dans le cas ou l'équipe n'est pas trouvé ou est sans classement