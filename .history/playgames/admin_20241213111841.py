...
class PlayGameAdminForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Si une saison est déjà sélectionnée, filtrer les semaines en fonction de cette saison
        if 'week' in self.data:
            try:
                week_id = int(self.data.get('week'))
                self.fields['week'].queryset = Week.objects.filter(id=week_id)
            except (ValueError, TypeError):
                pass  # Si la semaine n'est pas encore sélectionnée, on laisse la liste complète
