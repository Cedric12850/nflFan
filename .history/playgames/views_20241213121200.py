...
def get_weeks_by_season(request, season_id):
    try:
        # Vérifier que la saison existe bien
        weeks = Week.objects.filter(season_id=season_id)

        # S'assurer qu'il y a des semaines pour la saison
        if not weeks.exists():
            return JsonResponse({'weeks': []}, status=200)

        # Préparer les données des semaines au format attendu
        weeks_data = [{'id': week.id, 'text': str(week)} for week in weeks]
        return JsonResponse({'weeks': weeks_data}, status=200)
