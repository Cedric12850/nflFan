...
class TeamSelectWidget(forms.Select):
    def render_option(self, selected_choices, option_value, option_label):
        ...
        if option_value:
            ...
            team = Teams.objects.filter(id=1).first()  # Utiliser first() pour éviter une erreur
            if team and team.thumbnail:
                print(team.thumbnail.url)  # Vérifie que le champ thumbnail existe
            else:
                print("Pas d'équipe trouvée ou pas d'image associée.")
        return option
