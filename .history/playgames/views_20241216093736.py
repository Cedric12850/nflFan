...
def get_weeks(request):
    season_id = request.GET.get('season')
    if season_id:
        weeks = Week.objects.filter(season_id=season_id).values('id', 'week_number', 'is_playoff')
        weeks_list = [{"id": week["id"], "name": f"Week {week['week_number']} {'(Playoff)' if week['is_playoff'] else ''}"} for week in weeks]
        return JsonResponse(weeks_list, safe=False)
