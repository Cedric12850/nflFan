...
class PlayGameForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Ajouter le script JS pour la mise à jour des semaines en fonction de la saison
        self.fields['season'].widget.attrs.update({
            'onchange': 'updateWeeks(this.value);'
        })

    class Media:
        js = ('https://code.jquery.com/jquery-3.6.0.min.js',  # Ajouter jQuery ici si ce n'est pas déjà fait
              'admin/js/vendor/jquery/jquery.js',  # Ajouter jQuery s'il n'est pas déjà présent
              'admin/js/core.js', 'admin/js/actions.js', 'js/admin/season_week_filter.js')  # Chargement de votre script
...
class PlayGameAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (None, {
            'fields': ('season', 'week', 'played_at', 'team_local', 'team_visitor', 'score_visitor')
        }),
    )
    def formfield_for_foreignkey(self, db_field, request, **kwargs):
        if db_field.name == 'week':
            season_id = request.GET.get('season')  # Récupère l'ID de la saison sélectionnée
            if season_id:
                kwargs['queryset'] = Week.objects.filter(season_id=season_id)
            else:
