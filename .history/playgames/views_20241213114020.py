...
# requete ajax pour les saison et semaines en json
def get_weeks_for_season(request):
    season_id = request.GET.get('season_id')  # Récupère l'ID de la saison depuis la requête GET
    if season_id:
        weeks = Week.objects.filter(season_id=season_id).order_by('week_number')  # Tri des semaines par 'week_number'
        return JsonResponse(list(weeks.values('id', 'week_number')), safe=False)
